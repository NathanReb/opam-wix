...
We'll use `frama-c-eva`, a `frama-c`, `dune-site` based plugin as an example.
...
```
dune build @install -p foo-bar
dune install --relocatable -p foo-bar --prefix foo-bar.bundle
```
...
With this, the `oui` generated installer will install `foo-bar`'s bundle on the
